...
?>
    <div class="form-field term-utfb-id">
        <label for="embm_utfb_id">
            <?php _e('Untappd for Business ID', EMBM_DOMAIN); ?>
        </label>
    ...
    <div class="form-field term-sync-exclude">
        <label for="embm_sync_exclude">
            <input
                ...
                type="checkbox"
            >
            <span><?php _e('Exclude from Sync', EMBM_DOMAIN); ?></span>
        </label>
    </div>
<?php
}
...
function EMBM_Admin_Metabox_Menus_column($columns)
{
    ...
    if (!EMBM_Core_Beer_disabled()) {
        $columns['embm_utfb_id'] = __('Untappd for Business ID', EMBM_DOMAIN);
        $columns['embm_sync_exclude'] = __('Exclude from Sync', EMBM_DOMAIN);
    }
    return $columns;
}
